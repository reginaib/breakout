...
public class BreakoutState {
	...
	public BreakoutState(Ball[] balls, BlockState[] blocks, Point bottomRight, PaddleState paddle) {
		...
		if(!Arrays.stream(blocks).allMatch(b -> b != null)) throw new IllegalArgumentException();
		if(!Arrays.stream(blocks).allMatch(b -> getFieldInternal().contains(b.getLocation()))) throw new IllegalArgumentException();
		if(!Arrays.stream(balls).allMatch(b -> b != null)) throw new IllegalArgumentException();
	...
	public Ball[] getBalls() {
		Ball[] balls = new Ball[this.balls.length];
		for(int i = 0; i < balls.length; i++) {
			balls[i] = this.balls[i].clone();
		}
		return balls;
	}
	 ...
	 * @mutates | this
	 * @post | getPaddle().getCenter().getX() == old(getPaddle().getCenter().getX()) + Constants.PADDLE_VEL.getX() * elapsedTime
	 */
